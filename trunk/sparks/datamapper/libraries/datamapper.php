...
class DataMapper implements IteratorAggregate {
	...
	public function DataMapper($id = NULL)
	{
		...
		if ( ! isset(DataMapper::$common[$common_key]))
		{
			...
			if(! $loaded_from_cache)
			{
				...
				if(!isset($associative_validation[$this->primary_key]))
				{
					// label is set below, to prevent caching language-based labels
					$associative_validation[$this->primary_key] = array(
	...
	public function __get($name)
	{
		...
		if ($has_many || $has_one)
		{
			...
			// Store parent data
			$this->{$name}->parent = array('model' => $related_properties['other_field'], $this->primary_key => $this->{$this->primary_key});
	...
	{		
		...
		if(is_array($id))
		{
			$query = $this->db->where_in($this->primary_key, $id);
		}
		else
		{
	 ...
	 * First
	 *
	 * Get first objects from the database.
	 *
	 * @param	integer|NULL $limit Limit the number of results.
	 * @param	integer|NULL $offset Offset the results when limiting.
	 * @return	DataMapper Returns self for method chaining.
	...
	{
		$query = $this->db->order_by($this->primary_key, 'asc')->limit(1)->get($this->table);
	 ...
	 * Last
	 *
	 * Get last objects from the database.
	 *
	 * @param	integer|NULL $limit Limit the number of results.
	 * @param	integer|NULL $offset Offset the results when limiting.
	 * @return	DataMapper Returns self for method chaining.
	...
	public function save($object = '', $related_field = '')
	{
		...
		if ($this->valid)
		{
			...
			if ( ! empty($data))
			{
				if ( ! $this->_force_save_as_new && ! empty($data[$this->primary_key]))
				{
					// callback function after_save()
				...
				{
					// callback function before_save()
					if(method_exists($this, 'before_save'))
					{
						$this->before_save();
					}

					// Convert this object to array again
					$data = $this->_to_array();
		...
		foreach($objects as $index => $o)
		{
			...
			else
			{
				...
				if(isset($this->has_one[$rf]) && in_array($other_column, $this->fields))
				{
					...
					if($this->{$other_column} != $o->{$this->primary_key})
					{
						// ITFK: store on the table
						$this->{$other_column} = $o->{$this->primary_key};
	...
	public function delete($object = '', $related_field = '')
	{
		if (empty($object) && ! is_array($object))
		{
			...
			{
				// callback function before_delete()
					...
					foreach ($this->{$type} as $model => $properties)
					{
						...
						if ($properties['cascade_delete'])
						{
									...
									)
							{
								...
								$this->db->where($this_model . '_id', $this->{$this->primary_key});
								$this->db->update($object->table, $data);
							}
							else if ($relationship_table != $this->table)
							{

								$data = array($this_model . '_id' => $this->{$this->primary_key});
				...
				// callback function after_delete()
	...
	public function count($exclude_ids = NULL, $column = NULL, $related_id = NULL)
	{
		...
		if ( ! empty($this->parent))
		{
					...
					)
			{
				...
				$this->db->where($this->primary_key, $this->parent[$this->primary_key])->where($this_model . '_id IS NOT NULL');
			}
			else
			{
				// All other cases
				$this->db->where($relationship_table . '.' . $other_model . '_id', $this->parent[$this->primary_key]);
			}
	...
	public function exists()
	{
		...
		return isset($this->{$this->primary_key}) ? !empty($this->{$this->primary_key}) : ($this->result_count() > 0);
	}
	...
	protected function _handle_related()
	{
		if ( ! empty($this->parent))
		{
			...
			if ($has_many || $has_one)
			{
				if( ! $this->_get_relation($this->parent['model'], $this->parent[$this->primary_key]))
				{
					return FALSE;
				}
			}
	...
	private function _related($query, $arguments = array(), $extra = NULL)
	{
		if ( ! empty($query) && ! empty($arguments))
		{
			...
			if (is_object($arguments[0]))
			{
				...
				$field = (isset($arguments[1])) ? $arguments[1] : $this->primary_key;
				$value = (isset($arguments[2])) ? $arguments[2] : $object->{$this->primary_key};
				$next_arg = 3;
			}
			else
			{
				...
				if(isset($arguments[1]) && is_object($arguments[1]))
				{
					$object = $arguments[1];
					// Prepare field and value
					$field = (isset($arguments[2])) ? $arguments[2] : $this->primary_key;
					$value = (isset($arguments[3])) ? $arguments[3] : $object->{$this->primary_key};
					$next_arg = 4;
				}
				else
				{
			...
			if(preg_replace('/[!=<> ]/ ', '', $field) == $this->primary_key)
			{
	...
	public function is_related_to($related_field, $id = NULL)
	{
		...
		{
			$id = $related_field->{$this->primary_key};
	...
	public function include_related($related_field, $fields = NULL, $append_name = TRUE, $instantiate = FALSE)
	{
		...
		foreach ($fields as $field)
		{
			...
			if(in_array($new_field, $this->fields)) {
				if($instantiate && $field == $this->primary_key && $new_field != $this->primary_key) {
	...
	private function _get_relation($related_field, $id)
	{
		...
		$this->where_related($related_field, $this->primary_key, $id);
	...
	protected function _save_relation($object, $related_field = '')
	{
		...
		if ( ! empty($related_properties) && $this->exists() && $object->exists())
		{
			...
			{
				$this->{$other_model . '_id'} = $object->{$this->primary_key};
				$ret =  $this->save();
			...
			{
				$object->{$this_model . '_id'} = $this->{$this->primary_key};
				$ret = $object->save();
			...
			{
				$data = array($this_model . '_id' => $this->{$this->primary_key}, $other_model . '_id' => $object->{$this->primary_key});
				...
				if ($query->num_rows() == 0)
				{
					...
					if (isset($this->has_many[$related_field]))
					{
						...
						if (isset($object->has_one[$other_field]))
						{
							...
							$query = $this->db->get_where($relationship_table, array($other_model . '_id' => $object->{$this->primary_key}), 1, 0);

							if ($query->num_rows() > 0)
							{
								// Find and update the other objects existing relation to relate with this object
								$this->db->where($other_model . '_id', $object->{$this->primary_key});
						...
						else if (isset($object->has_many[$other_field]))
						{
							...
							{
								$data = array($this_model . '_id' => $object->{$this->primary_key}, $other_model . '_id' => $this->{$this->primary_key});
								$this->db->insert($relationship_table, $data);
							}
					...
					{
						// And it has an existing relation
						$query = $this->db->get_where($relationship_table, array($this_model . '_id' => $this->{$this->primary_key}), 1, 0);
	...
	private function _remove_other_one_to_one($rf, $object)
	{
		...
		$update->where($other_column, $object->{$this->primary_key});
		if($this->exists())
		{
			$update->where('id <>', $this->{$this->primary_key});
		}
	...
	protected function _delete_relation($object, $related_field = '')
	{
		...
		if ( ! empty($related_properties) && ! empty($this->{$this->primary_key}) && ! empty($object->{$this->primary_key}))
		{
			$this_model = $related_properties['join_self_as'];
			$other_model = $related_properties['join_other_as'];
	...
	protected function _count_related($related_field, $object = '')
	{
		...
		if ( ! empty($related_field) && ! empty($this->{$this->primary_key}))
		{
			...
			if( (!$one) || empty($ids))
			{
				...
				// Store parent data
				$object->parent = array('model' => $rel_properties['other_field'], $this->primary_key => $this->{$this->primary_key});
	...
	public function set_join_field($related_field, $field, $value = NULL, $object = NULL)
	{
		...
		else if (is_object($related_field))
		{
			$object = $related_field;
			$related_field = $object->model;
			$related_ids[] = $object->{$this->primary_key};
			$related_properties = $this->_get_related_properties($related_field);
		}
		else
		{
		...
		else
		{
			...
			{
				$this->db->where($this_model . '_id', $this->{$this->primary_key});
				$this->db->where($other_model . '_id', $obj->{$this->primary_key});
				$this->db->update($relationship_table, $field);
			}
		}
	...
	protected function _unique($field)
	{
		if ( ! empty($this->{$field}))
		{
			...
			if ($query->num_rows() > 0)
			{
				...
				if ($this->{$this->primary_key} != $row->{$this->primary_key})
				{
	...
	protected function _process_query($query)
	{
		if ($query->num_rows() > 0)
		{
			...
			$index = ($this->all_array_uses_ids && isset($this->{$this->primary_key})) ? $this->{$this->primary_key} : 0;
			...
			if($query->num_rows() > 1)
			{
				...
				foreach ($query->result() as $row)
				{
					...
					if($this->all_array_uses_ids && isset($item->{$this->primary_key}))
					{
						$this->all[$item->{$this->primary_key}] = $item;
					}
					else
					{
						$this->all[] = $item;
