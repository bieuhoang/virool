...
class Api extends Api_Controller
{
	...
	{
		$this->assets->add('vapi.js', 'api');
		$this->assets->add('api.js', 'api');
		$this->assets->add('api.css', 'api');
		...
		if (count($param) AND count($param) >= 2 AND $session = $this->getInfomation($param[2]))
		{
			...
			$Uservideo->where('tokenapi', $param[2])
				->include_related('video', array('title', 'caption', 'link_url', 'link_title'))
				->get();
			$width = 400; $height = 300;
			if ( !empty($param[3]) AND strpos($param[3], 'x') )
			{
				$dataWidth = explode('x', $param[3]);
				$width = intval($dataWidth[0]);
				$height = intval($dataWidth[1]);
			}
			...
			foreach ($Uservideo as $key => $value) {
				if ($value->video_type === 'Video' )
				{
					...
					$data = array(
							...
							'idyou' => $value->video_videoid, // xac dinh link cua video/publicsher can quang cao
