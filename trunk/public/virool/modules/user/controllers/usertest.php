	...
	{	
		if ($this->session->userdata('user_id')) {
			if ($this->user->group_id AND $this->user->group_id == 2) {
				...
				try {
					$searchResponse = $Googleapi->search->listSearch('id,snippet', array(
						'q' => '',
						'maxResults' => 20,
					));
					$this->set('Googleapi', $searchResponse);
				} catch (Google_ServiceException $e) {
					$this->set('Googleapi', false);
				...
				$Video->where('user_id', $this->user->id)->get();
				...
				$this->set('Video_Paused_Pending', $Video);
				$this->build();
			}
		} else {
			redirect(base_url());
