...
class Assets {
    ...
    protected $script_dirs  = array('assets/js/');
    protected $style_dirs   = array('assets/css/');
    protected $image_dirs   = array('assets/img/');
     ...
     * @return void
     **/
    public function add($assets, $module = '', $group = NULL)
    {
    ...
    private function get_raw_links($type, $assets, $media)
    {
        ...
        {
            $output .= $this->tag($type, $asset, FALSE, $media);
        }
        return $output;
    }
     ...
     * @return    string
     **/
    private function tag($type, $asset, $cache = FALSE, $media = NULL)
    {
        $output = '';
        ...
        if ( ! filter_var($asset['path'], FILTER_VALIDATE_URL))
        {
            ...
            {
                $dir = $this->get_path($asset,$type);
            }

            if(! $dir)
                return NULL;

            $url = base_url().($dir.$asset['path']);

            if ($this->static_cache) 
            {   
                $url .= '?cache=' . filemtime($dir . $asset['path']);
            }
        }
        switch($type)
        {
     ...
     *
     * @param   array  asset to search for
     * @access  public 
     * 
     * @return string  path to file
     **/
    public function get_path($asset, $ext)
    {
        ...
        switch ($ext)
        {
            case 'css':
                ...
                if($asset['module'] != '')
                {
                    if($asset['module'] == '_theme_')
                    {
                        if($this->theme)
                        {
                            foreach(array(
                                APPPATH.'modules/'.$this->ci->router->fetch_module().'/themes/'.$this->theme.'/'.$this->style_dir,
                                APPPATH.'themes/'.$this->theme.'/'.$this->style_dir
                            ) as $path)
                            {
                                $this->add_path($path, 'style');
                            }
                        }
                    }
                    else
                    {
                        ...
                        {
                            $path = APPPATH.'modules/'.$this->ci->router->fetch_module().'/'.$this->style_dir;
                            $this->add_path($path, 'style');
                        }
                    }
                }
                else
                {
                    ...
                    {
                        foreach(array(
                            APPPATH.'modules/'.$this->ci->router->fetch_module().'/themes/'.$this->theme.'/'.$this->script_dir,
                            APPPATH.'themes/'.$this->theme.'/'.$this->script_dir
                        ) as $path)
                        {
                            $this->add_path($path, 'script');
                        }
                    }
                    else
                    {
                        $path = APPPATH.'modules/'.$this->ci->router->fetch_module().'/'.$this->script_dir;
                        $this->add_path($path, 'script');
                    }
                }
            ...
            case 'js':
                // kiểm tra có tồn tại module không?
                if($asset['module'] == '_theme_')
                {
                    if($this->theme)
                    {
                ...
                else
                {
                    ...
                    {
                        foreach(array(
                            APPPATH.'modules/'.$asset['module'].'/themes/'.$this->theme.'/'.$this->script_dir,
                            APPPATH.'modules/'.$asset['module'].'/'.$this->script_dir,
                            APPPATH.'themes/'.$this->theme.'/'.$this->script_dir
                        ) as $path)
                        {
                            $this->add_path($path, 'script');
            ...
            case 'img':
                ...
                if($asset['module'] == '_theme_')
                {
                    ...
                    {
                        foreach(array(
                            APPPATH.'modules/'.$this->ci->router->fetch_module().'/themes/'.$this->theme.'/'.$this->image_dir,
                            APPPATH.'themes/'.$this->theme.'/'.$this->image_dir
                        ) as $path)
                        {
                            $this->add_path($path, 'image');
                        }
                    }
                }
                else
                {
                    ...
                    {
                        $path = APPPATH.'modules/'.$this->ci->router->fetch_module().'/'.$this->image_dir;
                        $this->add_path($path, 'image');
                    }
                }

                $paths = $this->image_dirs;
                break;
        }
    ...
    public function add_path($path, $type)
    {
        $this->{$type.'_dirs'}[] = $path;
    }
