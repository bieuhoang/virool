if(jQuery)(
                ...
                jQuery.extend(jQuery.fn,{
                        uploadify:function(options) {
                                ...
                                if (settings.onInit() !== false) {
                                        ...
                                        $.jsParams.fixTable("#" +jQuery(this).attr('id') + 'Queue');
                                }
                                ...
                                jQuery(this).bind("uploadifySelect", {'action': settings.onSelect, 'queueID': settings.queueID}, function(event, ID, fileObj) {
                                        if (event.data.action(event, ID, fileObj) !== false) {
                                                ...
                                                var option = [
                                                        ...
                                                        fileName,
                                                        (byteSize + suffix),
                                                        '<div class="uploadifyProgress"><div id="' + jQuery(this).attr('id') + ID + 'ProgressBar" class="uploadifyProgressBar"><!--Progress Bar--></div></div>',
                                                        '<a class="ahrefUploadFyCancel" href="javascript:jQuery(\'#' + jQuery(this).attr('id') + '\').uploadifyCancel(\'' + ID + '\')"></a>',
                                                        'tr'+ID

                                                ];
                                                $.jsParams.AddData(queue, option);
                                ...
                                jQuery(this).bind("uploadifyCancel", {'action': settings.onCancel}, function(event, ID, fileObj, data, remove, clearFast) {
                                        if (event.data.action(event, ID, fileObj, data, clearFast) !== false) {
                                                if(typeof($('tr.tr'+ID))!='undefined'){
                                                        $('tr.tr'+ID).remove();
                                                        $.jsParams.jsAutoIndex();
                                                }
                                                if (remove) {
        ...
        $.jsParams = {
                colModel : [
                        {'display': 'STT', 'name': 'stt', 'width':'50px', 'align':'center'},
                        {'display': 'Name', 'name': 'Name', 'width':'200px', 'align':'center'},
                        {'display': 'Size', 'name': 'Size', 'width':'120px', 'align':'center'},
                        {'display': ' ', 'name': 'Processing', 'width':'180px', 'align':'center'},
                        {'display': 'Action', 'name': 'action', 'width':'50px', 'align':'center'}
                ],
                ind : 0,
                fixTable : function(t ){ 
                    var that = this, idE = $(t), colRow = that.colModel, tbody = document.createElement('tbody');
                    idE.append('<table id="tabFixUp"></table>');
                    //thead
                    $('#tabFixUp').append('<thead></thead>')
                    var th = $('#tabFixUp thead');
                    var tr = document.createElement('tr');            
                    for (var i = 0; i < colRow.length; i++){            
                        var td = document.createElement('td');
                        var cm = colRow[i];
                        if(cm.display){
                            td.innerHTML = cm.display;
                        }
                        if(cm.align){
                            td.align = cm.align;
                        }
                        ...
                        if(cm.name){
                            $(td).attr('abbr', cm.name);
                        }
                        $(tr).append($(td));
                        $(th).append($(tr));
                    }
                    $('#tabFixUp').append(tbody);
                },
                AddData : function(t, option){
                    var that = this, tbody = $(t).children('table#tabFixUp').children('tbody');
                    if(option.length>0){
                        var tr = document.createElement('tr');
                ...
                jsAutoIndex : function(){
                        if(this.ind > 0){
                                this.ind --;
                        }
                        if(typeof($('td.tdRow1')) != 'undefined'){
                                $('td.tdRow1').each(function(i, e){
                                        $(this).text(i+1);
                                })
                        }
                }
