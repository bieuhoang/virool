...
class Template
{
	...
	{
		//set view detail if view not set
		$view = !$view 
			? ($this->_ci->router->fetch_class() . '/' . $this->_ci->router->fetch_method()) 
			: $view;
		...
		// add asset for module
		$this->_ci->load->library('assets');
		if($this->_module)
		{
			$this->_ci->assets->add($this->_controller.'.'.$this->_method.'.js', $this->_module);
		}
		else
		{
			$this->_ci->assets->add($this->_controller.'.'.$this->_method.'.js', '_theme_');
		}
		...
		foreach ($this->_partials as $name => $partial)
		{
			...
			if (isset($partial['view']) and $partial['view'] != '')
			{
	 ...
	 * @return	void
	 */
	public function layout($view = NULL, $_layout_subdir = '')
	{
		if(is_null($view))
		{
			return $this->_layout;
	 ...
	 * @return	void
	 */
	public function cache($minutes = 0)
	{
		...
		if ( ! empty($this->_theme))
		{
			foreach ($this->_theme_locations as $location)
			{
				$theme_views = array(
					...
					$this->_theme . '/modules/' . $this->_module . '/views/' . $view,
					'modules/' . $this->_module . '/themes/' . $this->_theme . '/views/' . $view,
					$this->_theme . '/views/' . $view
				);

				foreach ($theme_views as $theme_view)
				{
					$path = $location . $theme_view . self::_ext($theme_view);
					if(substr($theme_view, 0, 7) == 'modules') $path = APPPATH . $theme_view . self::_ext($theme_view);

					if (file_exists($path))
					{
						if(strpos($path, 'modules')) $location = APPPATH;
